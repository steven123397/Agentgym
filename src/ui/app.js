...
export const mountApp = (root) => {
  ...
  const renderToast = () => {
    if (!toast) return '';
    return `<div class="toast" role="status">${escapeHtml(toast)}</div>`;
  };
  ...
  const renderForm = () => {
    ...
    const editing =
      formMode === 'edit' ? tasks.find((t) => t.id === editingId) : null;
      ...
      <div class="welcome-card">
        <div class="welcome-brand">
          <span class="welcome-mark" aria-hidden="true">
            <svg viewBox="0 0 48 48" width="48" height="48" fill="none">
              ...
              <path d="M16 25.5 21.5 31 32 18" stroke="#fff" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round"/>
              <defs>
        ...
        <ul class="welcome-features">
          ...
          <li>
            ...
            <div>
              <strong>智能排序</strong>
              <span>高优先级优先，截止更近的先做</span>
            </div>
          </li>
          <li>
            <span class="welcome-icon" aria-hidden="true">③</span>
            <div>
              <strong>8 小时预算</strong>
              <span>装不下的任务会标明原因，不硬塞</span>
            </div>
          </li>
        </ul>
    ...
    <li class="task-card ${task.done ? 'is-done' : ''}" data-id="${escapeAttr(task.id)}">
      <label class="check">
        <input
          ...
          ${task.done ? 'checked' : ''}
          aria-label="标记完成：${escapeAttr(task.title)}"
        />
      </label>
      <div class="task-body">
  ...
  const render = () => {
    ...
    // 首次进入 / 清空后：全屏欢迎页；点「创建」后切到带表单的精简壳
    if (showWelcome) {
      root.innerHTML = `
